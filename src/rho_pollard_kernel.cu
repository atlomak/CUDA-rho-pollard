...
void zeromq_client(int32_t instances, volatile int32_t *warp_flags, volatile int32_t *stop_flag, EC_point *h_starting, EC_point *h_result);
...
__global__ void rho_pollard(cgbn_error_report_t *report, volatile rho_pollard_args args)
{
    ...
    cgbn_load(bn192_env, W.seed, &(args.starting[instance].seed));
    ...
    {
        while (!is_distinguish(bn192_env, W, args.parameters->zeros_count) && *args.stop_flag == 0)
        {
        ...
        __syncwarp();
        cgbn_store(bn192_env, &(args.result[instance].x), W.x);
        cgbn_store(bn192_env, &(args.result[instance].y), W.y);
        cgbn_store(bn192_env, &(args.result[instance].seed), W.seed);
        __threadfence_system();
        if (thread_id % 32 == 0)
        {
            args.warp_flags[warp_id] = 1;
            __threadfence_system();
        ...
        if (thread_id % 32 == 0)
        {
            while (args.warp_flags[warp_id] != 0) // While CPU set flat to 0, warp can continue
                ;
            __threadfence_system();
...
void run_rho_pollard(EC_point *startingPts, uint32_t instances, EC_point *precomputed_points, EC_parameters *parameters)
{
    ...
    EC_point *gpu_result;
    EC_point *h_result;
    cudaHostAlloc((void **)&h_result, sizeof(EC_point) * instances, cudaHostAllocMapped);
    cudaCheckErrors("Failed to allocate memory for result points");

    cudaHostGetDevicePointer(&gpu_result, h_result, 0);
    cudaCheckErrors("Failed to get device pointer for result points");
    ...
    *h_stop_flag = 0;
    ...
    zeromq_client(instances, h_warp_flags, h_stop_flag, h_starting, h_result);
    ...
    cudaCheckErrors("Failed to free memory for result points");
    ...
    while (*stop_flag == 0)
    {
        for (int i = 0; i < warps; i++)
        {
            if (warp_flags[i] == 1)
            {
                ...
                memcpy(distinguish_points, h_result + i * 8, sizeof(EC_point) * 8);
                zmq_send(requester, distinguish_points, sizeof(EC_point) * 8, 0);

                printf("CPU: Requesting new starting points\n");
                int msg_size = zmq_recv(requester, new_starting_points, sizeof(EC_point) * 8, 0);
                if (msg_size == 0)
                {
                    printf("CPU: Received stop signal\n");
                    *stop_flag = 1;
                    break;
                }
